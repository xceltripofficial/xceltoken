...
contract("XCELTOKEN", function(accounts) {
    ...
    it("creation: should create an initial balance of 30 bil for the public supply", function(done) {
        var ctr;
        XcelToken.new(tokenBuyerAddress, teamVestingAddress, teamVestingContractAddress).then(function(result) {
            ctr = result;
            return ctr.tokenBuyerAddr.call();
    }).then(function (result) {
        assert.strictEqual(result, tokenBuyerAddress);
    ...
    it("public supply purchase: token buyer should be able to send 10 xcel token to an address", function(done) {
        ...
        return ctr.buyTokens(testAddress, (new BigNumber(10)).times(new BigNumber(10).pow(18)), 'BTC', '0x4ed593e3b0f41cecd0de314c8e701361d3ad850f6bf252af4da9ef3a39fc6988',{from: tokenBuyerAddress});
      }).then(function (result) {
        console.log(result);
        return ctr.balanceOf(testAddress);
      }).then(function (result) {
